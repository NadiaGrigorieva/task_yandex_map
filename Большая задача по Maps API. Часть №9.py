...
class MapParams(object):
    ...
    def post(self, a):
        try:
            response = "http://geocode-maps.yandex.ru/1.x/"
            geocoder_params = {
                                "apikey": "40d1649f-0493-4b70-98ba-98533de7710b",
                                "geocode": a,
                                "format": "json",
                               }
            json_response = requests.get(response, params=geocoder_params)
...
def main():
    ...
    text4 = smallfont.render("", True, color)
    x4, y4 = 30, 450
    
    text3 = smallfont.render("Сброс", True, color)
    x3, y3 = 650, 180
    ...
    text5 = smallfont.render("", True, color)
    x5, y5 = 30, 490
    vv = False
    text6 = smallfont.render("OF", True, color)
    x6, y6 = 650, 300
    s = ""
    mp = MapParams()
    running = True
    while running:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if x2 <= mouse[0] <= x2 + 140 and y2 <= mouse[1] <= y2 + 40:
                mp.cen()
                fl = True
                text4 = smallfont.render(mp.geo(), True, color)
            if x3 <= mouse[0] <= x3 + 140 and y3 <= mouse[1] <= y3 + 40:
                ...
                text6 = smallfont.render("OF", True, color)
                vv = False
            if x6 <= mouse[0] <= x6 + 140 and y6 <= mouse[1] <= y6 + 40:
                if vv:
                    text5 = smallfont.render("", True, color)
                    text6 = smallfont.render("OF", True, color)
                    vv = False
                else:
                    text5 = smallfont.render(mp.post(mp.geo()), True, color)
                    text6 = smallfont.render("ON", True, color)
                    vv = True
        ...
        if x6 <= mouse[0] <= x6 + 140 and y6 <= mouse[1] <= y6 + 40:
            pygame.draw.rect(screen, color_light, [x6, y6, 140, 40])
        else:
            pygame.draw.rect(screen, color_dark, [x6, y6, 140, 40])
        screen.blit(text, (x, y))
        screen.blit(text1, (x1, y1))
        screen.blit(text2, (x2, y2))
        screen.blit(text3, (x3, y3))
        screen.blit(text4, (x4, y4))
        screen.blit(text5, (x5, y5))
        screen.blit(text6, (x6, y6))
        pygame.display.flip()
    os.remove(map_file)
